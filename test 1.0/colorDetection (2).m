...
max_angle = 45;
min_angle = -45;
max_pos = (90 + max_angle)/180;
min_pos = (90 + min_angle)/180;
...
tic
    ...
    % gray = frame(:,:,1)*R + frame(:,:,2)*G + frame(:,:,3)*B;
    % subtract_im = imsubtract(frame(:,:,1), gray);
    % subtract_im = imsubtract(frame(:,:,1), rgb2gray(frame));  %1-Red, 2-Green, 3-Blue
    subtract_im = frame(:,:,1)- rgb2gray(frame);
